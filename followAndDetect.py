...
if __name__ == "__main__":
    ...
    # adjusts drone to face, based on result CSV file of detector
    def adjustDroneToFace():
        ...
        # open csv file, make reader to find bounding box, works on LAST ROW
        with open(resultsPath, newline='') as csvfile:
            reader = csv.reader(csvfile, delimiter=',', quotechar='|')
            line_count = 0
            for row in reader:
                line_count += 1
                if line_count == row_length and row_length != 1:
                    x_min = row[2]
                    print("xMin " + x_min)
                    y_min = row[3]
                    print("yMin " + y_min)
                    ...
                    print("Mask detected, centering drone....")
                    center_drone(float(x_min), float(x_max), float(y_min), float(y_max), float(img_width),
                                 float(img_height))


    # centers drone to face, based on values given of mask bounding box
    def center_drone(x_min, x_max, y_min, y_max, img_width, img_height):
        ...
        # normal value between -0.5 and 0.5, 0 is when drone centered, positive is when face is above drone
        y_center = ((img_height - ((y_min + y_max) / 2)) / img_height) - 0.5
        ...
        if x_center > .1:
            mambo.fly_direct(roll=30, pitch=0, yaw=0, vertical_movement=0, duration=.2)
    ...
    if success:
        ...
        if success:
            ...
            mambo.smart_sleep(1)
            mambo.fly_direct(roll=0, pitch=0, yaw=0, vertical_movement=50, duration=.5)
            mambo.smart_sleep(2)
